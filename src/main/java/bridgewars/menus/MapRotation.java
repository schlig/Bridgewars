	...
	public static void sendInput(Player p, Inventory inv, ItemStack button) throws IOException {
		...
		case BARRIER:
			...
			if(value.contains("0")) {
				...
				f.write("1".getBytes());
				Bukkit.broadcastMessage(Message.chat("&6&l" + mapName + "&r has been " + "&cremoved" + " &rfrom the map rotation."));
				p.playSound(p.getLocation(), Sound.CLICK, 0.8F, 1F);
			}
			else {
				button = updateMap(button, false);
				f.write("0".getBytes());
				Bukkit.broadcastMessage(Message.chat("&6&l" + mapName + "&r has been " + "&aadded" + " &rto the map rotation."));
